...
<body style="height: 100vh; display: flex; flex-direction: column; overflow: hidden;">
  ...
  <div class="col-9 border-left custom-form">
      ...
      <?php
            ...
            $sql = "SELECT * FROM patients WHERE idPatient=".$_GET['idPatient'];
            $resultado = $conexion->query($sql);
            $row = $resultado->fetch_assoc();
            
      ?>
      <input type="hidden" class="form-control" name="idUsuario" value="<?php echo $row['idPatient'] ?>">
      <div class="row g-3">
      <div class="col-sm-6">
      ...
      <input name="direccionUsuario" type="text" class="form-control" 
       value="<?php echo $row['addressP']?>" disabled readonly>
      </div>
      <div class="col-sm-3">
        <label id="telefono" class="form-label">Telefono</label>
        <input name="telefonoUsuario" type="number" class="form-control" value="<?php echo $row['phoneNumber']?>" 
        disabled readonly>
      </div>
      <div class="col-sm-3">
        <label id="correo" class="form-label">Correo</label>
        <input name="correoUsuario" type="email" class="form-control" value="<?php echo $row['email']?>" 
        disabled readonly>
      </div>
      <div class="col-sm-3">
        <label id="profesion" class="form-label">Profesion</label>
        <input name="profesionUsuario" type="text" class="form-control" value="<?php echo $row['profession']?>" 
        disabled readonly>
      </div>
      <div class="col-sm-3">
        <label id="profesion" class="form-label">Genero</label>
      ...
      <div class="py-4">
        ...
        href="../PersonalMedicoPacientes.php">Volver</a>
      </div>
      </div>
      </div>
      </div>
